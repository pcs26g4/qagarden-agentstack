...
def test_TC_NAVIGATION_01(page):
    page.goto(CONSOLIDATED_LOCATORS_STATE_36_5D50_URL, timeout=60000)
...
@pytest.mark.regression
def test_TC_SEARCH_02(page):
    page.goto(CONSOLIDATED_LOCATORS_STATE_36_5D50_URL, timeout=60000)
    page.wait_for_load_state('networkidle')
    page.locator('#q').first.fill("")
    page.locator('input.btn.btn-primary').first.click()
...
def test_TC_RECORDS_PER_PAGE_01(page):
    page.goto(CONSOLIDATED_LOCATORS_STATE_36_5D50_URL, timeout=60000)
    page.wait_for_load_state('networkidle')
    page.locator('#per_page').first.select_option(value="50")