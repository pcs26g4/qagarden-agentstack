import sys
import re
import pytest
from playwright.sync_api import sync_playwright, expect
from pathlib import Path
ROOT_DIR = Path(__file__).resolve().parent.parent.parent
sys.path.insert(0, str(ROOT_DIR))

from config.urls import PAGE_27_URL
from playwright.sync_api import expect
import re
import pytest


@pytest.mark.smoke
def test_page_27_title(page):
    page.goto(PAGE_27_URL)
    page.wait_for_load_state('networkidle')
    expect(page).to_have_title(re.compile(r"Hockey Teams"))


@pytest.mark.regression
def test_page_27_search_team(page):
    page.goto(PAGE_27_URL)
    page.wait_for_load_state('networkidle')
    search_input = page.locator("#q")
    search_input.fill("Avalanche")
    page.locator('input.btn.btn-primary').first.click()
    expect(page.locator("xpath=//*[@id=\"hockey\"]/div[1]/table[1]/tbody[1]/tr[2]/td[1]").first).to_have_text("Colorado Avalanche")


@pytest.mark.regression
def test_page_27_pagination(page):
    page.goto(PAGE_27_URL)
    page.wait_for_load_state('networkidle')
    page.locator("xpath=//*[contains(@aria-label, \"Next\")]").click()
    page.wait_for_load_state('networkidle')
    expect(page).to_have_url(re.compile(r"page_num=16"))


@pytest.mark.regression
def test_page_27_change_per_page(page):
    page.goto(PAGE_27_URL)
    page.wait_for_load_state('networkidle')
    page.locator("#per_page").first.select_option("50")
    page.wait_for_load_state('networkidle')
    expect(page.locator('xpath=//*[@id="hockey"]/div[1]/div[1]/div[1]/h1[1]/small[1]').first).to_have_text("25 items")